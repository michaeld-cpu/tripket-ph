/* ============================================================================
 * Tripket — Build "Settings · Configurations — All states"
 * ----------------------------------------------------------------------------
 * Rebuilds ONLY the Configurations tab of app/settings/page.tsx, after its UX
 * rework: per-row Status switches, Class dropdowns, a real Accommodations
 * editor, and the "Vehicle types" rename. The Account and System tabs stay in
 * the `tripket-settings-allstates` plugin.
 *
 * The plugin CREATES its own section below every existing section on the page
 * and is ADDITIVE: a frame whose name already exists is skipped, never
 * replaced.
 *
 * Shares the chrome layer (sidebar, topbar, embedded logos, text measurement)
 * byte-for-byte with the other Tripket plugins. Configurations-only code starts
 * at the "K1." marker.
 *
 ...
 * py-2 = 8.5). Literal px classes do NOT scale — the 44px Status track, the
 * 170/180/120/140/100/36px grid tracks, and h-[22px] w-[38px] switches.
 * ========================================================================== */
...
/* ============================================================================
 * K1. Settings › Configurations — the reworked catalog UX
 * ----------------------------------------------------------------------------
 * Covers ONLY the Configurations tab of app/settings/page.tsx. The Account and
 * System tabs live in the `tripket-settings-allstates` plugin.
 *
 * What changed in this rework (vs. the earlier Configurations frames):
 *   · every editor gained a leading 44px Status column — RowStatusToggle,
 *     an EMERALD switch with a check glyph in the knob
 *   · Vehicle + Passenger editors gained a "Class" <Select> bound to the fixed
 *     VEHICLE_CLASS_OPTIONS / PASSENGER_CLASS_OPTIONS vocabularies
 *   · Accommodations is no longer a placeholder — it's a real editor backed by
 *     catalog.accommodations (Status · Name · Capacity · Base fare)
 *   · "Vehicle classes" is now called "Vehicle types" (the section id is still
 *     `vehicle-classes`)
 *   · the Vehicle editor dropped Descriptor / Max (kg / m) and renamed its
 *     columns to Class · Name · Capacity in vessel · Free pax · Fare
 * ========================================================================== */

const CK = { violet50: '#F5F3FF', violet700: '#6D28D9', gray700: '#374151', rose700: '#BE123C' };
Object.keys(CK).forEach((k) => { if (C[k] === undefined) C[k] = CK[k]; });
...
const KI = {
  ...
  checkTiny:{ sw: 3.5,  d: '<path d="M5 12l5 5 9-11"/>' },
};
...
// CONFIG_SECTIONS — note the label changed but the id did not.
...
// lib/settings-data.ts — the fixed class vocabularies.
const PASSENGER_CLASS_OPTIONS = ['Regular', 'Student', 'Senior Citizen', 'PWD', 'Child', 'Infant', 'Free'];
const VEHICLE_CLASS_OPTIONS = ['Motor', 'Car', 'SUV', 'Van', '6-Wheeler', '10-Wheeler'];

// DEFAULT_VEHICLE_CLASSES — `classKey` is NOT seeded, so the Class Select
// renders its italic "Select" placeholder on every row out of the box.
const VEHICLE_ROWS = [
  { label: 'Motorcycle / Tricycle', cls: '', capacity: '1', freePax: '1', fare: '300',  on: true },
  { label: 'Car / SUV / Van',       cls: '', capacity: '2', freePax: '1', fare: '1500', on: true },
  { label: 'Pickup / AUV',          cls: '', capacity: '2', freePax: '1', fare: '1800', on: true },
  { label: 'Light Truck / Elf',     cls: '', capacity: '3', freePax: '1', fare: '3500', on: true },
  { label: 'Heavy Truck / Trailer', cls: '', capacity: '5', freePax: '2', fare: '6500', on: true },
  { label: 'Bus / Minibus',         cls: '', capacity: '6', freePax: '2', fare: '5000', on: true },
];

// DEFAULT_PASSENGER_TYPES — no classKey either; `is_active` is undefined, and
// the editor reads `p.is_active !== false`, so every seeded row is enabled.
const PASSENGER_ROWS = [
  { label: 'Senior Citizen',               cls: '', amount: '20',  on: true },
  { label: 'Person with Disability (PWD)', cls: '', amount: '20',  on: true },
  { label: 'Student',                      cls: '', amount: '10',  on: true },
  { label: 'Infant',                       cls: '', amount: '100', on: true },
];

// DEFAULT_ACCOMMODATIONS — new in this rework.
const ACCOMMODATION_ROWS = [
  { label: 'Economy',  capacity: '200', fare: '500',  on: true },
  { label: 'Tourist',  capacity: '80',  fare: '900',  on: true },
  { label: 'Business', capacity: '40',  fare: '1500', on: true },
];

const ADDON_ROWS = [
  { label: 'Extra Cabin Bag',      price: '150', on: true },
  { label: 'Onboard Meal Pack',    price: '250', on: true },
  { label: 'Priority Boarding',    price: '150', on: true },
  { label: 'Arrival Vehicle Wash', price: '300', on: true },
];


/* ── K2. Chrome ────────────────────────────────────────────────────────── */

const TAB_H = SP.s2_5 * 2 + lh(FS.t13);
const RAIL_W = 221;                                   // w-52
const CARD_PAD = SP.s6;                               // p-6

function configPageHeader(parent) {
...
function configTabs(parent, y) {
  ...
  TABS.forEach((t) => {
    ...
    if (on) rect(tab, 'Active underline', SP.s2, TAB_H - 1, w - SP.s2 * 2, 2,
      { bg: C.brand500, radius: RAD.full });
    x += w + SP.s1;
  });
  return bar;
...
function configRail(parent, x, y, activeId) {
  ...
  CONFIG_SECTIONS.forEach((s) => {
    ...
    if (on) rect(item, 'Active indicator', 0, (itemH - 17) / 2, 3, 17, { bg: C.brand500, radius: 1.5 });
...
function kCard(parent, x, y, w, title, subtitle) {
  ...
  return { card: card, bodyY: CARD_PAD + lh(FS.t15) + SP.s05 + sub.height + SP.s4,
           innerW: innerW, x: CARD_PAD };
}


/* ── K3. Editor primitives ─────────────────────────────────────────────── */

const CAT_INPUT_H = SP.s1_5 * 2 + lh(FS.t12_5) + 2;   // px-2.5 py-1.5 text-[12.5px]
const SEL_H = SP.s2 * 2 + lh(FS.sm) + 2;              // Select, default size
const CAT_ROW_PY = SP.s2;                             // py-2
const REMOVE_BTN = 29.75;                             // h-7 w-7
const SW_W = 38, SW_H = 22, SW_KNOB = 18;             // RowStatusToggle, literal px
...
function catalogHeader(parent, x, y, w, hint, used, total, addLabel) {
  ...
  const leftW = w - (countW + SP.s3 + addW) - SP.s3;
  ...
  icon(add, 'Icon · plus', KI.plusBold, SP.s2, (addH - 12.75) / 2, 12.75, C.brand700);
  ...
  return Math.max(lh(FS.t10_5) + SP.s1 + hintT.height, addH) + SP.s3;
}

/** RowStatusToggle — emerald when on, with a check glyph inside the knob. */
function rowStatusToggle(parent, x, y, on) {
  const f = frame(parent, 'RowStatusToggle · ' + (on ? 'On' : 'Off'), x, y, SW_W, SW_H,
    { bg: on ? C.emerald500 : C.slate300, radius: RAD.full });
  const knob = frame(f, 'Knob', on ? 18 : 2, (SW_H - SW_KNOB) / 2, SW_KNOB, SW_KNOB,
    { bg: C.white, radius: RAD.full });
  if (on) icon(knob, 'Icon · check', KI.checkTiny, (SW_KNOB - 10.625) / 2, (SW_KNOB - 10.625) / 2,
    10.625, C.emerald500);
  return f;
}

/** Column header strip. `sub` renders a second line (Capacity / in vessel). */
function catalogColumns(parent, x, y, cols) {
  let maxLines = 1;
  cols.forEach((c) => { if (c.sub) maxLines = 2; });
  const lineH = maxLines > 1 ? FS.t10 * 1.25 : lh(FS.t10);   // leading-tight
  const blockH = lineH * maxLines;
  let cx = x;
  cols.forEach((c) => {
    if (c.label) {
      const t = text(parent, 'Th · ' + c.label, c.label.toUpperCase(), 0, y,
        { size: FS.t10, weight: FONT.semibold, color: C.slate400, tracking: 1.1, lh: lineH });
      t.x = c.right ? cx + c.w - t.width : cx;
      if (c.sub) {
        const s = text(parent, 'Th · ' + c.sub, c.sub.toUpperCase(), 0, y + lineH,
          { size: FS.t10, weight: FONT.semibold, color: C.slate400, tracking: 1.1, lh: lineH });
        s.x = c.right ? cx + c.w - s.width : cx;
      }
    }
    cx += c.w + SP.s2;
  });
  hairline(parent, 'Border bottom', x, y + blockH + SP.s2, cx - SP.s2 - x, C.slate100);
  return blockH + SP.s2 + 1;                          // pb-2 + border-b
}
...
/** ₱-prefixed fare box — px-2 py-1.5, peso sign left, value right. */
function catFareBox(parent, x, y, w, value) {
  const f = frame(parent, 'Input · ₱' + value, x, y, w, CAT_INPUT_H,
    { bg: C.white, radius: RAD.md, stroke: C.slate200 });
  text(f, 'Peso', '₱', SP.s2, (CAT_INPUT_H - lh(FS.t11)) / 2,
    { size: FS.t11, weight: FONT.medium, color: C.slate400 });
  const t = text(f, 'Value', value, 0, SP.s1_5 + 1, { size: FS.t12_5, color: C.slate900 });
...
/**
 * The Class <Select>, rendered `inline`. `value` is "" on every seeded row, so
 * the trigger shows the italic grey placeholder. When open, the menu is
 * absolutely positioned inside the wrapper (inline mode), so it draws over the
 * rows beneath it — queued and flushed last, since Figma has no z-index.
 */
let K_PENDING = [];
function classSelect(parent, x, y, w, value, options, open) {
  const f = frame(parent, 'Select · ' + (value || 'Select'), x, y, w, SEL_H,
    { bg: C.white, radius: RAD.lg, stroke: open ? C.gray300 : C.gray200 });
  const t = text(f, value ? 'Value' : 'Placeholder', value || 'Select', SP.s3, SP.s2 + 1,
    { size: FS.sm, color: value ? C.gray900 : C.gray400 });
  if (!value) t.fontName = { family: FONT.family, style: FONT.italic || FONT.regular };
  const chev = icon(f, 'Icon · chevron', I.chevronDown, w - SP.s3 - 14.875,
    (SEL_H - 14.875) / 2, 14.875, C.gray400);
  if (open) chev.rotation = 180;
  if (!open) return f;

  const optH = SP.s2 * 2 + lh(FS.sm);
  const menuH = Math.min(options.length * optH + SP.s1 * 2 + 2, 255);   // max-h-60
  K_PENDING.push(function () {
    const menu = frame(parent, 'Select menu · Class', x, y + SEL_H + SP.s1, w, menuH,
    ...
    options.forEach((opt, i) => {
      ...
      text(row, 'Label', opt, SP.s3, SP.s2, { size: FS.sm, color: C.gray700 });
    });
  });
  return f;
}
...
function removeButton(parent, x, y) {
  ...
  icon(b, 'Icon · trash', KI.trash, (REMOVE_BTN - 14.875) / 2, (REMOVE_BTN - 14.875) / 2,
    14.875, C.slate400);
  return b;
}

/** Resolve a CSS grid-template-columns spec (px numbers or {fr}). */
...
function emptyCatalog(parent, x, y, w, copy) {
  const t = text(parent, 'Empty', copy, x, y + SP.s3,
    { size: FS.t11_5, color: C.slate400, width: w, align: 'CENTER' });
  return SP.s3 * 2 + t.height;                        // py-6
}


/* ── K4. The four editors ──────────────────────────────────────────────── */

function vehicleEditor(parent, x, y, w, rows, o) {
  o = o || {};
  const used = rows.filter((r) => r.label.trim()).length;
  let cy = y + catalogHeader(parent, x, y, w,
    'Define vehicle types with capacity, default fare, and passenger limits.',
    used, rows.length, 'Add type');

  const cw = gridCols([44, 170, { fr: 1.4 }, 120, 100, 120, 36], w, SP.s2);
  cy += catalogColumns(parent, x, cy, [
    { label: 'Status', w: cw[0] },
    { label: 'Class', w: cw[1] },
    { label: 'Name', w: cw[2] },
    { label: 'Capacity', sub: 'in vessel', w: cw[3], right: true },
    { label: 'Free pax', w: cw[4], right: true },
    { label: 'Fare', w: cw[5], right: true },
    { label: '', w: cw[6] },
  ]);

  if (!rows.length) {
    return cy + emptyCatalog(parent, x, cy, w,
      'No vehicle types. Add one to make it available to vessels.') - y;
  }
  const rowH = CAT_ROW_PY * 2 + Math.max(SEL_H, CAT_INPUT_H, SW_H, REMOVE_BTN);
  rows.forEach((r, i) => {
    if (i > 0) hairline(parent, 'Divider', x, cy, w, C.slate100);
    const ry = cy + (i > 0 ? 1 : 0) + CAT_ROW_PY;
    const mid = (h) => ry + (rowH - CAT_ROW_PY * 2 - h) / 2;
    let cx = x;
    rowStatusToggle(parent, cx, mid(SW_H), r.on);                                cx += cw[0] + SP.s2;
    classSelect(parent, cx, mid(SEL_H), cw[1], r.cls, VEHICLE_CLASS_OPTIONS, o.openRow === i);
    cx += cw[1] + SP.s2;
    catInput(parent, cx, mid(CAT_INPUT_H), cw[2], r.label, 'e.g. ≤ 300kg GVW');  cx += cw[2] + SP.s2;
    catInput(parent, cx, mid(CAT_INPUT_H), cw[3], r.capacity, '', { right: true, mono: true });
    cx += cw[3] + SP.s2;
    catInput(parent, cx, mid(CAT_INPUT_H), cw[4], r.freePax, '', { right: true, mono: true });
    cx += cw[4] + SP.s2;
    catFareBox(parent, cx, mid(CAT_INPUT_H), cw[5], r.fare);                     cx += cw[5] + SP.s2;
    removeButton(parent, cx + (cw[6] - REMOVE_BTN) / 2, mid(REMOVE_BTN));
    cy += rowH + (i > 0 ? 1 : 0);
  });
  return cy - y;
}

function passengerEditor(parent, x, y, w, rows, o) {
  o = o || {};
  const used = rows.filter((r) => r.label.trim()).length;
  let cy = y + catalogHeader(parent, x, y, w,
    'Set the class and discount for each fare category. Mark Infant if the row is the free-fare / no-seat type.',
    used, rows.length, 'Add type');

  const cw = gridCols([44, 180, { fr: 1.4 }, 170, 36], w, SP.s2);
  cy += catalogColumns(parent, x, cy, [
    { label: 'Status', w: cw[0] },
    { label: 'Class', w: cw[1] },
    { label: 'Name', w: cw[2] },
    { label: 'Discount (% or ₱)', w: cw[3], right: true },
    { label: '', w: cw[4] },
  ]);

  if (!rows.length) {
    return cy + emptyCatalog(parent, x, cy, w,
      'No passenger types. Add one to make it available to vessels.') - y;
  }
  const DISC_H = SP.s9;                                // h-9
  const rowH = CAT_ROW_PY * 2 + Math.max(SEL_H, DISC_H, CAT_INPUT_H, SW_H, REMOVE_BTN);
  rows.forEach((r, i) => {
    if (i > 0) hairline(parent, 'Divider', x, cy, w, C.slate100);
    const ry = cy + (i > 0 ? 1 : 0) + CAT_ROW_PY;
    const mid = (h) => ry + (rowH - CAT_ROW_PY * 2 - h) / 2;
    let cx = x;
    rowStatusToggle(parent, cx, mid(SW_H), r.on);                                cx += cw[0] + SP.s2;
    classSelect(parent, cx, mid(SEL_H), cw[1], r.cls, PASSENGER_CLASS_OPTIONS, o.openRow === i);
    cx += cw[1] + SP.s2;
    catInput(parent, cx, mid(CAT_INPUT_H), cw[2], r.label, 'e.g. Senior Citizen'); cx += cw[2] + SP.s2;

    // Discount = [₱/% segmented] + [number] + ["off"]
    const pct = !(o.flatRow === i);
    const box = frame(parent, 'Discount', cx, mid(DISC_H), cw[3], DISC_H,
      { bg: C.white, radius: RAD.md, stroke: C.slate200 });
    ...
    icon(unit, 'Icon · chevron', KI.chevDown, symW - SP.s1 - 12.75, (DISC_H - 12.75) / 2, 12.75, C.slate400);
    rect(box, 'Border right', symW, 0, 1, DISC_H, { bg: C.slate200 });
    const offW = SP.s2 + measure('OFF', FS.t10_5, FONT.medium, 0.63);
    text(box, 'Off', 'OFF', cw[3] - offW, (DISC_H - lh(FS.t10_5)) / 2,
    ...
    amt.x = cw[3] - offW - SP.s2 - amt.width;
    cx += cw[3] + SP.s2;
    removeButton(parent, cx + (cw[4] - REMOVE_BTN) / 2, mid(REMOVE_BTN));
    cy += rowH + (i > 0 ? 1 : 0);
  });
  return cy - y;
}

function accommodationsEditor(parent, x, y, w, rows) {
  const used = rows.filter((r) => r.label.trim()).length;
  let cy = y + catalogHeader(parent, x, y, w,
    'Define accommodation types with seat capacity and default fare. Vessels can toggle availability but cannot override the catalog values.',
    used, rows.length, 'Add accommodation');

  const cw = gridCols([44, { fr: 1 }, 140, 140, 36], w, SP.s2);
  cy += catalogColumns(parent, x, cy, [
    { label: 'Status', w: cw[0] },
    { label: 'Name', w: cw[1] },
    { label: 'Capacity', w: cw[2], right: true },
    { label: 'Base fare', w: cw[3], right: true },
    { label: '', w: cw[4] },
  ]);

  if (!rows.length) {
    return cy + emptyCatalog(parent, x, cy, w,
      'No accommodations. Add one to make it available to vessels.') - y;
  }
  const rowH = CAT_ROW_PY * 2 + Math.max(CAT_INPUT_H, SW_H, REMOVE_BTN);
  rows.forEach((r, i) => {
    if (i > 0) hairline(parent, 'Divider', x, cy, w, C.slate100);
    const ry = cy + (i > 0 ? 1 : 0) + CAT_ROW_PY;
    const mid = (h) => ry + (rowH - CAT_ROW_PY * 2 - h) / 2;
    let cx = x;
    rowStatusToggle(parent, cx, mid(SW_H), r.on);                                 cx += cw[0] + SP.s2;
    catInput(parent, cx, mid(CAT_INPUT_H), cw[1], r.label, 'e.g. Economy');       cx += cw[1] + SP.s2;
    catInput(parent, cx, mid(CAT_INPUT_H), cw[2], r.capacity, '', { right: true, mono: true });
    cx += cw[2] + SP.s2;
    catFareBox(parent, cx, mid(CAT_INPUT_H), cw[3], r.fare);                      cx += cw[3] + SP.s2;
    removeButton(parent, cx + (cw[4] - REMOVE_BTN) / 2, mid(REMOVE_BTN));
    cy += rowH + (i > 0 ? 1 : 0);
  });
  return cy - y;
...
function addOnsEditor(parent, x, y, w, rows) {
  ...
  const cw = gridCols([44, { fr: 1 }, 140, 36], w, SP.s2);
  cy += catalogColumns(parent, x, cy, [
    { label: 'Status', w: cw[0] },
    { label: 'Name', w: cw[1] },
  ...
  rows.forEach((r, i) => {
    ...
    let cx = x;
    rowStatusToggle(parent, cx, mid(SW_H), r.on);                                 cx += cw[0] + SP.s2;
    catInput(parent, cx, mid(CAT_INPUT_H), cw[1], r.label, 'e.g. Extra Cabin Bag'); cx += cw[1] + SP.s2;
    catFareBox(parent, cx, mid(CAT_INPUT_H), cw[2], r.price);                     cx += cw[2] + SP.s2;
    removeButton(parent, cx + (cw[3] - REMOVE_BTN) / 2, mid(REMOVE_BTN));
    cy += rowH + (i > 0 ? 1 : 0);
  });
  return cy - y;
}
...
function buildSaveBar(parent, disabledHint) {
  ...
  text(bar, 'Label', disabledHint || 'You have unsaved changes',
    SP.s8 + 6.375 + SP.s2, cy + (btnH - lh(FS.t12_5)) / 2,
  ...
  if (disabledHint) save.opacity = 0.6;
...
const CARDS = {
  ...
  'vehicle-classes': {
    title: 'Vehicle types',
    subtitle: 'Define the vehicle catalog for this shipping line. Vessels toggle which types they accept; edits here flow live into every vessel that uses them.',
  },
  'add-ons': {
    title: 'Add-ons',
...
function configShell(name, x, y) {
  ...
  buildTopbar(right);
  const main = frame(right, 'Main Content', 0, TOPBAR_H, MAIN_W, MAIN_H, { clip: true });
  const content = frame(main, 'Container', CONTENT_X, CONTENT_Y, CONTENT_W, 2400);
  return { frame: f, content: content };
}

function buildConfigPage(name, x, y, o) {
  o = o || {};
  K_PENDING = [];
  const section = o.section || 'passenger-types';
  const s = configShell(name, x, y);
  const head = configPageHeader(s.content);
  const tabsY = head.height + SP.s6;
  configTabs(s.content, tabsY);
  const bodyY = tabsY + TAB_H + SP.s5;

  configRail(s.content, 0, bodyY + SP.s4, section);    // sticky top-4

  const colX = RAIL_W + SP.s6;                         // gap-6
  const colW = CONTENT_W - colX - SP.s1;               // pr-1 gutter
  const spec = CARDS[section];
  const c = kCard(s.content, colX, bodyY, colW, spec.title, spec.subtitle);

  let bodyH = 0;
  if (section === 'passenger-types') {
    bodyH = passengerEditor(c.card, c.x, c.bodyY, c.innerW,
      o.rows || PASSENGER_ROWS, { openRow: o.openRow, flatRow: o.flatRow });
  } else if (section === 'vehicle-classes') {
    bodyH = vehicleEditor(c.card, c.x, c.bodyY, c.innerW,
      o.rows || VEHICLE_ROWS, { openRow: o.openRow });
  } else if (section === 'accommodations') {
    bodyH = accommodationsEditor(c.card, c.x, c.bodyY, c.innerW, o.rows || ACCOMMODATION_ROWS);
  } else {
    bodyH = addOnsEditor(c.card, c.x, c.bodyY, c.innerW, o.rows || ADDON_ROWS);
  }
  // Open Select menus paint last — Figma order IS paint order.
  K_PENDING.forEach((draw) => draw());
  K_PENDING = [];
  c.card.resize(colW, c.bodyY + bodyH + CARD_PAD);
...
const PAX_BLANK = PASSENGER_ROWS.slice(0, 3).concat([{ label: '', cls: '', amount: '0', on: true }]);
const PAX_CLASSED = [
  { label: 'Senior Citizen',               cls: 'Senior Citizen', amount: '20',  on: true },
  { label: 'Person with Disability (PWD)', cls: 'PWD',            amount: '20',  on: true },
  { label: 'Student',                      cls: 'Student',        amount: '10',  on: false },
  { label: 'Infant',                       cls: 'Infant',         amount: '100', on: true },
];
const VEH_CLASSED = [
  { label: 'Motorcycle / Tricycle', cls: 'Motor',      capacity: '1', freePax: '1', fare: '300',  on: true },
  { label: 'Car / SUV / Van',       cls: 'Car',        capacity: '2', freePax: '1', fare: '1500', on: true },
  { label: 'Pickup / AUV',          cls: 'SUV',        capacity: '2', freePax: '1', fare: '1800', on: true },
  { label: 'Light Truck / Elf',     cls: '6-Wheeler',  capacity: '3', freePax: '1', fare: '3500', on: false },
  { label: 'Heavy Truck / Trailer', cls: '10-Wheeler', capacity: '5', freePax: '2', fare: '6500', on: true },
  { label: 'Bus / Minibus',         cls: 'Van',        capacity: '6', freePax: '2', fare: '5000', on: true },
];

const BUILDERS = [
  { name: 'Config / 01 — Passenger types — Seeded',
    build: (x, y, n) => buildConfigPage(n, x, y, { section: 'passenger-types' }) },

  { name: 'Config / 02 — Vehicle types — Seeded',
    build: (x, y, n) => buildConfigPage(n, x, y, { section: 'vehicle-classes' }) },

  { name: 'Config / 03 — Accommodations',
    build: (x, y, n) => buildConfigPage(n, x, y, { section: 'accommodations' }) },

  { name: 'Config / 04 — Add-ons',
    build: (x, y, n) => buildConfigPage(n, x, y, { section: 'add-ons' }) },

  { name: 'Config / 05 — Passenger class select open',
    build: (x, y, n) => buildConfigPage(n, x, y, { section: 'passenger-types', openRow: 0 }) },

  { name: 'Config / 06 — Vehicle class select open',
    build: (x, y, n) => buildConfigPage(n, x, y, { section: 'vehicle-classes', openRow: 0 }) },

  { name: 'Config / 07 — Passenger types — Classes assigned, one row off',
    build: (x, y, n) => buildConfigPage(n, x, y, {
      section: 'passenger-types', rows: PAX_CLASSED, saveBar: true }) },

  { name: 'Config / 08 — Vehicle types — Classes assigned, one row off',
    build: (x, y, n) => buildConfigPage(n, x, y, {
      section: 'vehicle-classes', rows: VEH_CLASSED, saveBar: true }) },

  { name: 'Config / 09 — Discount unit — Flat ₱',
    build: (x, y, n) => buildConfigPage(n, x, y, {
      section: 'passenger-types', rows: PAX_CLASSED, flatRow: 1 }) },

  { name: 'Config / 10 — Blank label — Save disabled',
    build: (x, y, n) => buildConfigPage(n, x, y, {
      section: 'passenger-types', rows: PAX_BLANK,
      saveBar: true, saveDisabledHint: 'Every catalog row needs a label' }) },

  { name: 'Config / 11 — Empty catalog — Accommodations',
    build: (x, y, n) => buildConfigPage(n, x, y, { section: 'accommodations', rows: [] }) },

  { name: 'Config / 12 — Empty catalog — Vehicle types',
    build: (x, y, n) => buildConfigPage(n, x, y, { section: 'vehicle-classes', rows: [] }) },
];


/* ── K8. Entry point ───────────────────────────────────────────────────── */

const SECTION_NAME = 'Settings · Configurations — All states';
