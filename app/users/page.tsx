...
export default function UsersPage() {
  ...
  useEffect(() => { setPage(1); }, [query, lineFilter, roleFilter, statusFilter]);

  const filtered = useMemo(() => {
    if (!users) return [];
    const q = query.trim().toLowerCase();
    return users.filter((u) => {
      if (lineFilter !== "all" && u.lineId !== lineFilter) return false;
      if (roleFilter !== "all" && u.role !== roleFilter) return false;
      if (statusFilter !== "all" && u.status !== statusFilter) return false;
      if (q && !`${u.name} ${u.email}`.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [users, query, lineFilter, roleFilter, statusFilter]);
      ...
      {!users ? (
        ...
        <section className="rounded-2xl bg-white shadow-[0_20px_40px_-24px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/70">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-t-2xl border-b border-slate-100 px-5 py-4">
            <div>
              <h2 className="text-base font-semibold tracking-tight text-slate-900">Platform users</h2>
              <p className="mt-0.5 text-xs text-slate-500">
                Showing <span className="font-medium text-slate-900">{filtered.length}</span> of {users.length} users across {lines.length} shipping lines
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Select size="sm" value={lineFilter} onChange={setLineFilter} ariaLabel="Filter by shipping line" className="w-48" options={lineOptions} />
              <Select
                size="sm"
                value={roleFilter}
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 align-middle">
                        <div className="flex items-center gap-2">
                          {line && <LogoTile line={line} size={20} />}
                          <span className="truncate text-[12.5px] font-medium tracking-tight text-slate-700">{line?.name ?? u.lineId}</span>
                        </div>
                      </td>
          ...
          <Pagination page={page} pageSize={PAGE_SIZE} total={filtered.length} onPageChange={setPage} noun="users" />
        </section>
      )}
