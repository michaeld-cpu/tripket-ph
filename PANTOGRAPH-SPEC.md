# Pantograph — product spec (draft)

> **pantograph** *(n.)* — the drafting linkage that reproduces a drawing exactly,
> at any scale. You trace the original; the far arm draws a true copy. No
> interpretation, no freehand.

**Your interface, redrawn — not screenshotted.**

One Figma plugin, installed once, that never changes. A prompt document a
designer pastes into whatever agent they use. The agent reads their codebase and
writes a `scene.json`. The designer drops that file on the plugin and gets native
Figma frames — real text nodes, real vectors, real geometry.

The name carries the promise: **faithful, not approximate.** Everything here is
transcribed from source, never eyeballed from a screenshot.
...
| **`scene.json`** | the agent, per project | the scene description that gets uploaded |

Two reasons this matters. First, a designer handed a file called `manifest.json`
will reasonably try to install it as a plugin and hit an error. Second, **Figma's
brand guidelines restrict using "Figma" or lookalike "Fig-" constructions in a
product name**, and Community review is where that surfaces — which is why this
is Pantograph and not Figscene. Worth re-checking against their current
...
```
   PANTOGRAPH.md                    scene.json                  the plugin
   ─────────                    ────────────                  ──────────
...
2. **`PANTOGRAPH.md`** — what the designer pastes. Teaches the agent the schema and
...
## 4. `PANTOGRAPH.md` — the designer's half
...
```
pantograph validate scene.json     → schema + theme errors, exit 1 on failure
pantograph render   scene.json     → SVG per screen, for eyeballing
pantograph probe    scene.json     → node geometry dump, for overlap assertions
```
...
| 6 | `PANTOGRAPH.md`, docs, Tripket reference theme | the actual product |
...
3. **Auto-layout can't do everything.** Some real layouts genuinely need
   ...
   tail.
4. **Theme drift.** Nothing stops a `scene.json` theme block from going stale
   against the project's Tailwind config. A `pantograph theme --from tailwind.config.ts`
...
- **Multi-file input.** One `scene.json` per module, or one big file per
