 ...
 * ----------------------------------------------------------------------------
 * Rebuilds app/vessels/page.tsx as native Figma frames. The plugin CREATES its
 * own section below every existing section on the page, so nothing already in
 * the file is touched.
 *
 * Third plugin in the set, after Tickets and Routes. It shares the chrome layer
 * with both (sidebar, topbar, page header, modal shell, embedded logos, text
 * measurement); vessels-only code starts at the "V1." marker.
 *
...
/* ── V1. Vessels tokens, icons, seed data ──────────────────────────────── */
...
// Sizes the shared FS table doesn't carry.
const FS_V = { t14: 15, t19: 20 };

// leading-relaxed (1.625) vs the 1.5 default.
const RELAXED = 1.625;

// statusTone — app/vessels/page.tsx. Active is the only opaque "healthy"
// signal; Maintenance borrows brand-orange; Inactive and Retired share a
// muted slate, so the pill alone can't tell them apart (see the README).
const V_TONE = {
  Active:      { bg: C.emerald100, fg: C.emerald800, label: 'Active'      },
  Inactive:    { bg: C.slate100,   fg: C.slate500,   label: 'Inactive'    },
  Maintenance: { bg: C.brand50,    fg: C.brand700,   label: 'Maintenance' },
  Retired:     { bg: C.slate100,   fg: C.slate500,   label: 'Retired'     },
};

// statusDescription — the Tooltip content on each status pill.
const V_STATUS_HINT = {
...
const VI = {
  editVessel:    { sw: 1.75, d: '<path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/>' },
  disableVessel: { sw: 1.75, d: '<circle cx="12" cy="12" r="9"/><path d="M5.6 5.6l12.8 12.8"/>' },
  enableVessel:  { sw: 1.75, d: '<path d="M20 6 9 17l-5-5"/>' },
  copy:          { sw: 1.75, d: '<rect x="9" y="9" width="11" height="11" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h10"/>' },
  ...
  // The ferry glyph the Add/Edit dialog header and the fleet empty state share.
  ferryHeader:   { sw: 1.75, d: '<path d="M12 3v3"/><path d="M6 13V8h12v5"/><path d="M3 13h18l-1.8 5.2a2 2 0 0 1-1.9 1.3H6.7a2 2 0 0 1-1.9-1.3L3 13Z"/><path d="M2.5 21c1.2 0 1.2-1 2.4-1s1.2 1 2.4 1 1.2-1 2.4-1 1.2 1 2.3 1 1.2-1 2.4-1 1.2 1 2.4 1 1.2-1 2.3-1"/>' },
  fleet:         { sw: 1.5,  d: '<path d="M3 17h18l-2 3H5l-2-3Z"/><rect x="5" y="11" width="14" height="6" rx="1"/><path d="M8 11V7h8v4"/><path d="M12 7V4"/>' },
  lock:          { sw: 1.75, d: '<rect x="4" y="11" width="16" height="9" rx="1.5"/><path d="M8 11V8a4 4 0 0 1 8 0v3"/>' },
};

/**
 * The 2GO fleet from `lib/dashboard-data.ts`, in seed order.
 *
 * `n` is `shortVesselId(v.id)` — an FNV-1a hash of the id, mod 100 — so the ID
 * column is neither the array index nor anything stable across a re-seed. It's
 * reproduced here verbatim rather than invented; see the README.
 * `ini` is `vesselInitials(v.name)` for the avatar tile.
 */
const VESSELS = [
  { n: 56, imo: '9756101', name: 'MV Palawan Breeze',        ini: 'PB', type: 'RoRo', st: 'Active'      },
  { n: 13, imo: '9112401', name: 'MV Visayan Star',          ini: 'VS', type: 'RoRo', st: 'Active'      },
  { n: 94, imo: '9223511', name: 'MV St. Pope John Paul',    ini: 'SP', type: 'RoRo', st: 'Active'      },
  { n: 51, imo: '9334622', name: 'MV Maligaya',              ini: 'MA', type: 'RoRo', st: 'Active'      },
  { n: 32, imo: '9445702', name: 'MV Our Lady of Akita',     ini: 'OL', type: 'RoRo', st: 'Maintenance' },
  { n: 89, imo: '9445701', name: 'MV St. Therese of Lisieux',ini: 'ST', type: 'RoRo', st: 'Active'      },
  { n: 70, imo: '9556801', name: 'MV St. Joan of Arc',       ini: 'SJ', type: 'RoRo', st: 'Active'      },
  { n: 23, imo: '9445809', name: 'MV St. Augustine',         ini: 'SA', type: 'RoRo', st: 'Retired'     },
  { n:  4, imo: '9223621', name: 'MV St. Anthony',           ini: 'SA', type: 'RoRo', st: 'Active'      },
];

// `query` matches name OR imo, case-insensitively on the name only.
const VESSELS_SEARCH = VESSELS.filter((v) => v.name.indexOf('MV St.') === 0);

const TYPE_OPTIONS = ['All types', 'RoRo', 'Fast Craft'];
const STATUS_OPTIONS = ['All status', 'Active', 'Inactive'];
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];

// VesselFormBody typeOptions — primary line over the parenthesised descriptor.
const VESSEL_TYPE_CARDS = [
  { primary: 'RoRo',           secondary: 'Roll-on / Roll-off' },
  { primary: 'Fast Craft',     secondary: 'Passenger only'     },
  { primary: 'Passenger Ship', secondary: 'Passenger only'     },
];

// ADD_STATUSES / EDIT_STATUSES — both dialogs trim the four-status set to two.
const FORM_STATUSES = ['Active', 'Inactive'];

/* ── V2. Table metrics (px-5 py-3.5; thead py-2.5) ─────────────────────── */

const V_PAD_X = SP.s5;                                   // px-5
const V_PAD_Y = SP.s3_5;                                 // py-3.5
const V_AVATAR = SP.s9;                                  // h-9 w-9
const V_ROW_H = V_PAD_Y * 2 + V_AVATAR;                  // the avatar is the tallest cell
const V_THEAD_H = SP.s2_5 * 2 + lh(FS.t11);              // py-2.5
const V_ACTIONS_W = V_PAD_X * 2 + 29.75;                 // w-10 cell + kebab
const V_TOOLBAR_H = SP.s4 * 2 + (SP.s1_5 * 2 + lh(FS.sm));  // py-4 around py-1.5 controls

/** Auto table layout at px-5, mirroring how the browser sizes these columns. */
function vLayout(defs, rows) {
  let x = 0;
  const cols = defs.map((d) => {
    let w = measure(d.label.toUpperCase(), FS.t11, FONT.medium, 0.96);
    if (d.sortable) w += SP.s1_5 + 12.75;
    rows.forEach((r) => { w = Math.max(w, d.width(r)); });
    const col = { key: d.key, label: d.label, sortable: d.sortable, x: x, w: w + V_PAD_X * 2 };
    x += col.w;
    return col;
  });
  const natural = x + V_ACTIONS_W;
  // `w-full` stretches the table when the natural total falls short — which is
  // what happens here, unlike the routes table.
  if (natural < CONTENT_W) {
    const slack = CONTENT_W - natural;
    let acc = 0;
    cols.forEach((c) => {
      const share = slack * (c.w / x);
      c.x += acc; acc += share; c.w += share;
    });
    return { cols: cols, width: CONTENT_W };
  }
  return { cols: cols, width: natural };
}

/* ── V3. Toolbar — h2 + search + two size-sm Selects ───────────────────── */

const V_SEARCH_W = SP.s3 * 2 + 17 + SP.s2 + 204;        // px-3 + icon + gap + w-48
const V_TYPE_W = 153;                                    // w-36
const V_STATUS_W = 170;                                  // w-40
const V_CTRL_H = SP.s1_5 * 2 + lh(FS.sm);                // py-1.5, size="sm"

function selectSm(parent, name, x, y, w, label, open) {
  const f = frame(parent, name, x, y, w, V_CTRL_H, {
    bg: C.white, radius: RAD.lg, stroke: open ? C.gray300 : C.gray200,
  });
  if (open) rect(f, 'Focus ring', -2, -2, w + 4, V_CTRL_H + 4,
    { bg: C.brand100, radius: RAD.lg, opacity: 0.7 });
  text(f, 'Value', label, SP.s3, (V_CTRL_H - lh(FS.sm)) / 2, { size: FS.sm, color: C.gray900 });
  const ch = icon(f, 'Icon · chevron', I.chevronDown, w - SP.s3 - 12.75,
    (V_CTRL_H - 12.75) / 2, 12.75, C.gray400);
  if (open) ch.rotation = 180;
  return f;
}

function buildVesselsToolbar(card, opts) {
  const o = opts || {};
  const tb = frame(card, 'Toolbar', 0, 0, card.width, V_TOOLBAR_H);
  hairline(tb, 'Border bottom', 0, V_TOOLBAR_H - 1, card.width, C.slate100);
  text(tb, 'Toolbar title', 'Registered vessels', V_PAD_X, (V_TOOLBAR_H - lh(FS.base)) / 2,
    { size: FS.base, weight: FONT.semibold, color: C.slate900, tracking: -0.3 });

  const cy = (V_TOOLBAR_H - V_CTRL_H) / 2;
  const statusX = card.width - V_PAD_X - V_STATUS_W;
  const typeX = statusX - SP.s2 - V_TYPE_W;
  const searchX = typeX - SP.s2 - V_SEARCH_W;

  const sf = frame(tb, 'Search field', searchX, cy, V_SEARCH_W, V_CTRL_H, {
    bg: C.white, radius: RAD.lg, stroke: o.query ? C.gray300 : C.gray200,
  });
  if (o.query) rect(sf, 'Focus ring', -2, -2, V_SEARCH_W + 4, V_CTRL_H + 4,
    { bg: C.brand100, radius: RAD.lg, opacity: 0.7 });
  icon(sf, 'Icon · search', I.search, SP.s3, (V_CTRL_H - 17) / 2, 17, C.gray400);
  text(sf, o.query ? 'Query' : 'Placeholder', o.query || 'Search vessel or IMO',
    SP.s3 + 17 + SP.s2, (V_CTRL_H - lh(FS.sm)) / 2,
    { size: FS.sm, color: o.query ? C.gray900 : C.gray400 });

  const tsel = selectSm(tb, 'Select - Filter by type', typeX, cy, V_TYPE_W,
    o.type || 'All types', o.typeOpen);
  const ssel = selectSm(tb, 'Select - Filter by status', statusX, cy, V_STATUS_W,
    o.status || 'All status', o.statusOpen);
  return { tb: tb, typeSel: tsel, statusSel: ssel, cy: cy };
}

/** Portaled Select menu — floats over the page, so it is drawn on the frame. */
function selectMenu(parent, x, y, w, options, current) {
  const optH = SP.s2 * 2 + lh(FS.sm);
  const H = SP.s2 + options.length * optH + SP.s2;
  const menu = frame(parent, 'Select options', x, y, w, H, {
    bg: C.white, radius: RAD.xl, stroke: C.gray200, clip: true, shadow: MENU_SHADOW,
  });
  options.forEach((opt, i) => {
    const on = opt === current;
    const row = frame(menu, 'Option · ' + opt, 0, SP.s2 + i * optH, w, optH,
      { bg: on ? C.brand50 : undefined });
    text(row, 'Label', opt, SP.s3, SP.s2, { size: FS.sm, color: on ? C.brand700 : C2.gray700 });
    if (on) icon(row, 'Icon · check', VI.selectCheck, w - SP.s3 - 17, (optH - 17) / 2, 17, C.brand600);
  });
  return menu;
}
...
function vStatusPillW(st) {
  return measure(V_TONE[st].label.toUpperCase(), FS.t10, FONT.semibold, 0.88) + SP.s2 * 2;
}
function vTypePillW(t) {
  return measure(t, FS.t11, FONT.medium) + SP.s2 * 2;
}

const VESSEL_COL_DEFS = [
  { key: 'n',    label: 'ID',
    width: (v) => measure(String(v.n), FS.t13) },
  { key: 'imo',  label: 'IMO',
    width: (v) => measure(v.imo, FS.t13, FONT.semibold) + SP.s1_5 + SP.s5 },
  { key: 'st',   label: 'Status',
    width: (v) => vStatusPillW(v.st) },
  { key: 'name', label: 'Vessel', sortable: true,
    width: (v) => V_AVATAR + SP.s3 + measure(v.name, FS.sm, FONT.medium, -0.3) },
  { key: 'type', label: 'Type',
    width: (v) => vTypePillW(v.type) },
];

/** Status pill — rounded-md px-2 py-0.5, uppercase, tracking-[0.08em]. */
function vStatusPill(parent, x, y, st) {
  const label = V_TONE[st].label.toUpperCase();
  const w = vStatusPillW(st);
  const h = lh(FS.t10) + SP.s05 * 2;
  const pill = frame(parent, 'Status pill', x, y, w, h, { bg: V_TONE[st].bg, radius: RAD.md });
  text(pill, 'Label', label, SP.s2, SP.s05,
    { size: FS.t10, weight: FONT.semibold, color: V_TONE[st].fg, tracking: 0.88 });
  return pill;
}

/** Type pill — rounded-full px-2 py-0.5, bordered, no fill. */
function vTypePill(parent, x, y, t) {
  const w = vTypePillW(t);
  const h = lh(FS.t11) + SP.s05 * 2;
  const pill = frame(parent, 'Type pill', x, y, w, h,
    { radius: RAD.full, stroke: C.gray200 });
  text(pill, 'Label', t, SP.s2, SP.s05, { size: FS.t11, weight: FONT.medium, color: C2.gray600 });
  return pill;
}

/** Avatar tile — h-9 w-9 rounded-lg bg-brand-100, bold brand-600 initials. */
function vAvatar(parent, x, y, ini) {
  const a = frame(parent, 'Vessel avatar', x, y, V_AVATAR, V_AVATAR,
    { bg: C.brand100, radius: RAD.lg });
  const t = text(a, 'Initials', ini, 0, 0,
    { size: FS.xs, weight: FONT.bold, color: C.brand600 });
  centerIn(t, { x: 0, y: 0, w: V_AVATAR, h: V_AVATAR });
  return a;
}

function buildVesselsTable(parent, y, opts) {
  const o = opts || {};
  const rows = o.rows || [];
  // The empty row is a single td with px-5 py-12.
  const bodyH = rows.length ? rows.length * V_ROW_H : 51 * 2 + lh(FS.sm);
  const cardH = V_TOOLBAR_H + V_THEAD_H + bodyH + PAGER_H;
  const L = vLayout(VESSEL_COL_DEFS, rows.length ? rows : VESSELS);
  const cols = L.cols;

  const card = frame(parent, 'Card - Registered vessels', 0, y, CONTENT_W, cardH, {
    bg: C.white, radius: RAD.xxl, stroke: C.slate200, strokeOpacity: 0.7, shadow: CARD_SHADOW, clip: true,
  });
  const bar = buildVesselsToolbar(card, o);

  const table = frame(card, 'Table', 0, V_TOOLBAR_H, CONTENT_W, V_THEAD_H + bodyH);

  // border-y — this thead has a rule above and below, unlike the routes one.
  const thead = frame(table, 'Table header', 0, 0, CONTENT_W, V_THEAD_H,
    { bg: C.slate50, opacity: 0.5 });
  hairline(thead, 'Border top', 0, 0, CONTENT_W, C.slate100);
  hairline(thead, 'Border bottom', 0, V_THEAD_H - 1, CONTENT_W, C.slate100);
  cols.forEach((c) => {
    const label = c.label.toUpperCase();
    const lt = text(thead, 'Header ' + c.label, label, c.x + V_PAD_X, SP.s2_5,
      { size: FS.t11, weight: FONT.medium, color: C.slate500, tracking: 0.96 });
    if (c.sortable) {
      icon(thead, 'Icon · sort', VI.sortUpDown, c.x + V_PAD_X + lt.width + SP.s1_5,
        SP.s2_5 + (lh(FS.t11) - 12.75) / 2, 12.75, C.gray300);
    }
  });

  const tbody = frame(table, 'Table body', 0, V_THEAD_H, CONTENT_W, bodyH);
  if (!rows.length) {
    const msg = text(tbody, 'No results', 'No vessels match your filters.', 0, 0,
      { size: FS.sm, color: C.slate400, width: CONTENT_W, align: 'CENTER' });
    msg.y = (bodyH - msg.height) / 2;
  }

  rows.forEach((v, i) => {
    const row = frame(tbody, 'Row · ' + v.name, 0, i * V_ROW_H, CONTENT_W, V_ROW_H);
    if (i > 0) hairline(row, 'Divider', 0, 0, CONTENT_W, C.slate100);
    if (o.menuRowIndex === i || o.hoverRowIndex === i) {
      row.fills = fill(C.slate50, 0.6);
      rect(row, 'Hover accent', 0, 0, 3, V_ROW_H, { bg: C.brand500 });
    }

    text(row, 'Id', String(v.n), cols[0].x + V_PAD_X, (V_ROW_H - lh(FS.t13)) / 2,
      { size: FS.t13, color: C.slate500 });

    // IMO + its copy affordance, which swaps to an emerald check for ~1.4s.
    const imoT = text(row, 'IMO', v.imo, cols[1].x + V_PAD_X, (V_ROW_H - lh(FS.t13)) / 2,
      { size: FS.t13, weight: FONT.semibold, color: C.slate900 });
    const copied = o.copiedRowIndex === i;
    const btn = frame(row, copied ? 'Copied' : 'Button - Copy IMO',
      cols[1].x + V_PAD_X + imoT.width + SP.s1_5, (V_ROW_H - SP.s5) / 2, SP.s5, SP.s5,
      { bg: copied ? undefined : (o.copyHoverIndex === i ? C.slate100 : undefined), radius: RAD.md });
    icon(btn, 'Icon', copied ? VI.copied : VI.copy, (SP.s5 - 14.875) / 2, (SP.s5 - 14.875) / 2,
      14.875, copied ? C.emerald600 : C.slate400);

    const sh = lh(FS.t10) + SP.s05 * 2;
    vStatusPill(row, cols[2].x + V_PAD_X, (V_ROW_H - sh) / 2, v.st);

    vAvatar(row, cols[3].x + V_PAD_X, V_PAD_Y, v.ini);
    text(row, 'Vessel name', v.name, cols[3].x + V_PAD_X + V_AVATAR + SP.s3,
      (V_ROW_H - lh(FS.sm)) / 2,
      { size: FS.sm, weight: FONT.medium, color: C.slate900, tracking: -0.3 });

    const th = lh(FS.t11) + SP.s05 * 2;
    vTypePill(row, cols[4].x + V_PAD_X, (V_ROW_H - th) / 2, v.type);

    kebab(row, CONTENT_W - V_ACTIONS_W + V_PAD_X, (V_ROW_H - 29.75) / 2, o.menuRowIndex === i);
  });

  const pagerY = V_TOOLBAR_H + V_THEAD_H + bodyH;
  buildVesselsPager(card, pagerY, {
  ...
  return { card: card, bar: bar, cols: cols, pagerY: pagerY };
}

/* ── V5. Pager with the rows-per-page control ──────────────────────────── */

/**
 * Same Pagination component the routes table uses: the footer always renders
 * because it carries the running count, and only the Previous/chips/Next nav
 * collapses at one page. The 2GO fleet is nine vessels, so that collapsed state
 * is the default here.
 */
function buildVesselsPager(card, y, opts) {
  const o = opts || {};
...
/**
 * Two items only. The second is chosen by `status === "Active"`, so every
 * non-Active state — Inactive, Maintenance, Retired — offers **Enable vessel**.
 * `tone` is never passed here, so both items render in the default slate
 * except Disable, which still uses the legacy `danger` shorthand.
 */
function vesselMenuItems(st) {
  const items = [{ label: 'Edit vessel', ic: VI.editVessel }];
  if (st === 'Active') items.push({ label: 'Disable vessel', ic: VI.disableVessel, danger: true });
  else items.push({ label: 'Enable vessel', ic: VI.enableVessel });
  return items;
}

function buildVesselMenu(parent, triggerX, triggerY, items) {
  const W = 221;                                    // w-52
  ...
  items.forEach((it, i) => {
    const tile = frame(menu, 'Menu item · ' + it.label, SP.s1, SP.s1 + i * ITEM_H, W - SP.s2, ITEM_H,
      { radius: RAD.md });
    icon(tile, 'Icon', it.ic, SP.s2_5, (ITEM_H - 17) / 2, 17, it.danger ? C.rose500 : C.slate400);
    text(tile, 'Label', it.label, SP.s2_5 + 17 + SP.s2_5, SP.s1_5,
      { size: FS.t13, weight: FONT.medium, color: it.danger ? C.rose600 : C.slate700 });
  });
  return menu;
}

function vesselMenuAnchor(shell, rowIndex) {
  return {
    x: MAIN_X + CONTENT_X + CONTENT_W - V_ACTIONS_W + V_PAD_X,
    y: TOPBAR_H + CONTENT_Y + shell.bodyY + V_TOOLBAR_H + V_THEAD_H
       + rowIndex * V_ROW_H + (V_ROW_H - 29.75) / 2,
  };
}

/** Tooltip bubble — slate-900 pill above the trigger with a caret. */
function buildTooltip(parent, cx, bottomY, label) {
  const h = SP.s1_5 * 2 + lh(FS.t11);
  const w = SP.s2_5 * 2 + measure(label, FS.t11, FONT.medium);
  const tip = frame(parent, 'Tooltip · ' + label, cx - w / 2, bottomY - h - 8, w, h,
    { bg: C.slate900, radius: RAD.md, shadow: MENU_SHADOW });
  text(tip, 'Label', label, SP.s2_5, SP.s1_5,
    { size: FS.t11, weight: FONT.medium, color: C.white });
  const caret = rect(parent, 'Tooltip caret', cx - 4, bottomY - 8 - 3, 7, 7,
    { bg: C.slate900, radius: 1 });
  caret.rotation = 45;
  return tip;
}

/* ── V7. AddVesselModal / EditVesselModal — one shared body ────────────── */

const FORM_W = 714;                                  // max-w-2xl → 42rem × 17px
const FORM_BODY_MAX = 540;                           // max-h-[60vh] of 900

function formField(parent, x, y, w, label, required, control, hint) {
  const labelH = lh(FS.t13) + SP.s1_5;               // mb-1.5
  const lt = text(parent, 'Field label', label, x, y,
    { size: FS.t13, weight: FONT.medium, color: C.slate700, tracking: -0.3 });
  if (required) text(parent, 'Required', '*', x + lt.width + SP.s05, y,
    { size: FS.t13, weight: FONT.medium, color: C.brand600 });
  const ch = control(parent, x, y + labelH, w);
  let h = labelH + ch;
  if (hint) {
    text(parent, 'Field hint', hint, x, y + h + SP.s1,
      { size: FS.t11, color: C.slate500, width: w });
    h += SP.s1 + lh(FS.t11);
  }
  return h;
}

function textInput(parent, x, y, w, value, placeholder, mono) {
  const h = SP.s2 * 2 + lh(FS.sm);
  const f = frame(parent, value ? 'Input' : 'Input - empty', x, y, w, h,
    { bg: C.white, radius: RAD.lg, stroke: C.slate200 });
  text(f, value ? 'Value' : 'Placeholder', value || placeholder, SP.s3, SP.s2,
    { size: FS.sm, color: value ? C.slate900 : C.slate400 });
  return h;
}

/** LinePicker — a locked mirror of the top-bar switcher, not a real dropdown. */
function linePicker(parent, x, y, w) {
  const boxH = SP.s2_5 * 2 + Math.max(28, lh(FS.t12_5) + lh(FS.t10_5));
  const box = frame(parent, 'LinePicker', x, y, w, boxH,
    { bg: C.slate50, opacity: 0.6, radius: RAD.lg, stroke: C.slate200 });
  logoTile(box, SP.s2_5, (boxH - 28) / 2, 28);
  const tx = SP.s2_5 + 28 + SP.s2_5;
  const blockH = lh(FS.t12_5) + lh(FS.t10_5);
  text(box, 'Line name', LINE.name, tx, (boxH - blockH) / 2,
    { size: FS.t12_5, weight: FONT.semibold, color: C.slate900, tracking: -0.3 });
  text(box, 'Plan', 'Business plan', tx, (boxH - blockH) / 2 + lh(FS.t12_5),
    { size: FS.t10_5, color: C.slate500 });
  icon(box, 'Icon · lock', VI.lock, w - SP.s2_5 - 12.75, (boxH - 12.75) / 2, 12.75, C.slate400);

  const note = text(parent, 'Line notice',
    'To change line, switch Shipping Line first in the top bar.',
    x + SP.s1, y + boxH + SP.s1_5,
    { size: FS.t11, color: C.slate500, lh: FS.t11 * RELAXED, width: w - SP.s1 * 2 });
  note.setRangeFills(20, 33, fill(C.slate700));
  note.setRangeFontName(20, 33, { family: FONT.family, style: FONT.medium });
  return boxH + SP.s1_5 + lh(FS.t11) * RELAXED;
}

function typeCards(parent, x, y, w, picked) {
  const gap = SP.s2;
  const cw = (w - gap * 2) / 3;
  const h = SP.s2_5 * 2 + lh(FS.t13) + SP.s05 + FS.t11 * 1.25;
  VESSEL_TYPE_CARDS.forEach((c, i) => {
    const on = c.primary === picked;
    const card = frame(parent, 'Type card · ' + c.primary, x + i * (cw + gap), y, cw, h, {
      bg: on ? C.brand50 : C.white, opacity: on ? 0.4 : 1, radius: RAD.lg,
      stroke: on ? C.brand500 : C.slate200,
    });
    if (on) card.fills = fill(C.brand50, 0.4);
    text(card, 'Primary', c.primary, SP.s3, SP.s2_5,
      { size: FS.t13, weight: FONT.semibold, color: on ? C.brand700 : C.slate900, tracking: -0.3 });
    text(card, 'Secondary', c.secondary, SP.s3, SP.s2_5 + lh(FS.t13) + SP.s05,
      { size: FS.t11, color: C.slate500, lh: FS.t11 * 1.25, width: cw - SP.s3 * 2 });
  });
  return h;
}

function statusChips(parent, x, y, w, picked) {
  const h = SP.s1_5 * 2 + lh(FS.sm);
  let cx = x;
  FORM_STATUSES.forEach((s) => {
    const on = s === picked;
    const cw = SP.s3 * 2 + measure(s, FS.sm, FONT.medium);
    const chip = frame(parent, 'Status chip · ' + s, cx, y, cw, h,
      { bg: on ? C.brand50 : undefined, radius: RAD.md });
    text(chip, 'Label', s, SP.s3, SP.s1_5,
      { size: FS.sm, weight: FONT.medium, color: on ? C.brand700 : C.slate500 });
    cx += cw + SP.s1;
  });
  return h;
}
...
/**
 * Add and Edit are the same dialog: identical `VesselFormBody`, identical
 * footer, identical trimmed Active/Inactive status set. Only the header copy,
 * the IMO chip beside the title, and the submit label differ.
 *
 * opts: { mode: 'add' | 'edit', name, imo, type, status }
 */
function buildVesselFormDialog(parent, opts) {
  const o = opts || {};
  ...
  const H = headH + FORM_BODY_MAX + footH;

  const dlg = buildModal(parent, 'Dialog - ' + (isEdit ? 'Edit vessel' : 'Add new vessel'), W, H, false);

  /* Header — px-5 py-3.5 */
  const head = frame(dlg, 'Header', 0, 0, W, headH);
  hairline(head, 'Border bottom', 0, headH - 1, W, C.slate100);
  const badge = frame(head, 'Icon badge', SP.s5, (headH - 29.75) / 2, 29.75, 29.75,
    { bg: C.brand50, radius: RAD.lg });
  icon(badge, 'Icon', VI.ferryHeader, (29.75 - 17) / 2, (29.75 - 17) / 2, 17, C.brand600);
  const tx = SP.s5 + 29.75 + SP.s2_5;
  const blockH = lh(FS.t15) + lh(FS.t11_5);
  const ty = (headH - blockH) / 2;
  const title = text(head, 'Title', isEdit ? 'Edit vessel' : 'Add new vessel', tx, ty,
    { size: FS.t15, weight: FONT.semibold, color: C.slate900, tracking: -0.3 });
  if (isEdit) {
    // "Edit vessel · IMO 9756101" — the dot and mono ref sit on the title line.
    const dot = text(head, 'Separator', '·', tx + title.width + SP.s2, ty + lh(FS.t15) - lh(FS.t11),
      { size: FS.t11, color: C.slate400 });
    text(head, 'IMO ref', 'IMO ' + (o.imo || '—'),
      dot.x + dot.width + SP.s2, ty + lh(FS.t15) - lh(FS.t10),
      { size: FS.t10, color: C.slate500 });
  }
  // Both dialogs render STEP_COPY[1] — Edit included, even though the wizard
  // steps it refers to no longer exist. See the README.
  text(head, 'Caption', "Enter the vessel's basic identity details", tx, ty + lh(FS.t15),
    { size: FS.t11_5, color: C.slate500 });
  const close = frame(head, 'Button - Close', W - SP.s5 - 29.75, (headH - 29.75) / 2, 29.75, 29.75,
    { radius: RAD.md });
  icon(close, 'Icon', I.close, (29.75 - 14.875) / 2, (29.75 - 14.875) / 2, 14.875, C.slate400);

  /* Body — VesselFormBody, space-y-5 px-6 py-6.
     The form's natural height (~582) exceeds max-h-[60vh] (540 at this
     viewport), so it scrolls and the Status field falls below the fold.
     `o.scroll: 'bottom'` renders it scrolled to the end. */
  const body = frame(dlg, 'Body', 0, headH, W, FORM_BODY_MAX, { clip: true });
  const inner = frame(body, 'Form', 0, 0, W, FORM_BODY_MAX);
  const iw = W - SP.s6 * 2;
  let fy = SP.s6;
  const GAP = SP.s5;                                  // space-y-5

  fy += formField(inner, SP.s6, fy, iw, 'Shipping line', true,
    (p, x, y, w) => linePicker(p, x, y, w), 'Owning line for this vessel.') + GAP;

  fy += formField(inner, SP.s6, fy, iw, 'Vessel name', true,
    (p, x, y, w) => textInput(p, x, y, w, name, 'e.g. MV Palawan Breeze')) + GAP;

  fy += formField(inner, SP.s6, fy, iw, 'Vessel type', true,
    (p, x, y, w) => typeCards(p, x, y, w, o.type || 'RoRo')) + GAP;

  fy += formField(inner, SP.s6, fy, iw, 'IMO number', false,
    (p, x, y, w) => textInput(p, x, y, w, o.imo || '', 'e.g. 9756101', true),
    'Optional · 7-digit International Maritime Organization number') + GAP;

  rect(inner, 'Divider', SP.s6, fy, iw, 1, { bg: C.slate100, opacity: 0.7 });
  fy += 1 + GAP;

  fy += formField(inner, SP.s6, fy, iw, 'Status', false,
    (p, x, y, w) => statusChips(p, x, y, w, o.status || 'Active'));
  fy += SP.s6;                                        // py-6 bottom

  inner.resize(W, Math.max(fy, FORM_BODY_MAX));
  const overflow = Math.max(0, fy - FORM_BODY_MAX);
  if (overflow > 0) {
    if (o.scroll === 'bottom') inner.y = -overflow;
    // Scrollbar track — overflow-y-auto shows one while the body is scrollable.
    const trackH = FORM_BODY_MAX * (FORM_BODY_MAX / fy);
    const trackY = o.scroll === 'bottom' ? FORM_BODY_MAX - trackH : 0;
    rect(body, 'Scrollbar', W - 8, trackY + 2, 4, trackH - 4,
      { bg: C.slate300, radius: RAD.full, opacity: 0.8 });
  }

  /* Footer — px-6 py-4, justify-end gap-2 */
  const btnH = SP.s1_5 * 2 + lh(FS.sm);
  const foot = frame(dlg, 'Footer', 0, H - footH, W, footH);
  hairline(foot, 'Border top', 0, 0, W, C.slate100);
  const by = (footH - btnH) / 2;
  const label = isEdit ? 'Save changes' : 'Register vessel';
  const pw = SP.s3 * 2 + measure(label, FS.sm, FONT.medium);
  const cw = SP.s3 * 2 + measure('Cancel', FS.sm, FONT.medium);
  const ca = frame(foot, 'Button - Cancel', W - SP.s6 - pw - SP.s2 - cw, by, cw, btnH,
    { bg: C.white, radius: RAD.lg, stroke: C.slate200 });
  text(ca, 'Label', 'Cancel', SP.s3, SP.s1_5, { size: FS.sm, weight: FONT.medium, color: C.slate700 });
  const pr = frame(foot, 'Button - ' + label, W - SP.s6 - pw, by, pw, btnH,
    { bg: C.brand600, radius: RAD.lg });
  // disabled:opacity-60 — the button keeps its brand fill and dims.
  if (!valid) pr.opacity = 0.6;
  text(pr, 'Label', label, SP.s3, SP.s1_5, { size: FS.sm, weight: FONT.medium, color: C.white });
  return dlg;
}

/* ── V8. VesselStatusDialog (components/DeleteVesselDialog.tsx) ────────── */

const MD_W = 476;                                    // max-w-md → 28rem × 17px

function buildVesselStatusDialog(parent, disable) {
  const W = MD_W, PAD = SP.s6, badgeS = SP.s9;
  const vessel = disable ? 'MV Visayan Star' : 'MV Our Lady of Akita';
  const tail = disable
  ...
  const full = vessel + tail;

  const textW = W - PAD * 2 - badgeS - SP.s4;
  const lines = Math.max(1, Math.ceil(measure(full, FS.t13) / textW));
  const block = lh(FS.t15_5) + SP.s1_5 + lines * (FS.t13 * RELAXED);
  const footH = SP.s3_5 * 2 + (SP.s1_5 * 2 + lh(FS.sm));
  const H = PAD + Math.max(badgeS, block) + SP.s5 + footH;

  const dlg = buildModal(parent, 'Dialog - ' + (disable ? 'Disable' : 'Enable') + ' vessel', W, H, false);
  const badge = frame(dlg, 'Icon badge', PAD, PAD, badgeS, badgeS, {
    bg: disable ? C.rose50 : C.emerald50, radius: RAD.full,
    stroke: disable ? C.rose200 : C.emerald200, strokeOpacity: 0.7,
  });
  icon(badge, 'Icon', disable ? VI.disableVessel : VI.enableVessel,
    (badgeS - 18) / 2, (badgeS - 18) / 2, 18, disable ? C.rose600 : C.emerald600);

  const tx = PAD + badgeS + SP.s4;
  text(dlg, 'Title', disable ? 'Disable this vessel?' : 'Enable this vessel?', tx, PAD,
    { size: FS.t15_5, weight: FONT.semibold, color: C.slate900, tracking: -0.3 });
  const bd = text(dlg, 'Body', full, tx, PAD + lh(FS.t15_5) + SP.s1_5,
    { size: FS.t13, color: C.slate600, lh: FS.t13 * RELAXED, width: textW });
  bd.setRangeFills(0, vessel.length, fill(C.slate900));
  bd.setRangeFontName(0, vessel.length, { family: FONT.family, style: FONT.medium });

  const btnH = SP.s1_5 * 2 + lh(FS.sm);
  const foot = frame(dlg, 'Footer', 0, H - footH, W, footH);
  hairline(foot, 'Border top', 0, 0, W, C.slate100);
  const by = (footH - btnH) / 2;
  const pLabel = disable ? 'Disable vessel' : 'Enable vessel';
  const pw = SP.s3 * 2 + measure(pLabel, FS.sm, FONT.medium);
  const cw = SP.s3 * 2 + measure('Cancel', FS.sm, FONT.medium);
  const ca = frame(foot, 'Button - Cancel', W - PAD - pw - SP.s2 - cw, by, cw, btnH,
    { bg: C.white, radius: RAD.lg, stroke: C.slate200 });
  text(ca, 'Label', 'Cancel', SP.s3, SP.s1_5, { size: FS.sm, weight: FONT.medium, color: C.slate700 });
  const pr = frame(foot, 'Button - ' + pLabel, W - PAD - pw, by, pw, btnH,
    { bg: disable ? C.rose600 : C.emerald600, radius: RAD.lg });
  text(pr, 'Label', pLabel, SP.s3, SP.s1_5, { size: FS.sm, weight: FONT.medium, color: C.white });
  return dlg;
}

/* ── V9. EmptyState — kind="fleet" ─────────────────────────────────────── */

function buildFleetEmptyState(parent, y) {
  const H = 400;                                     // min-h-[400px]
  const panel = frame(parent, 'EmptyState - fleet', 0, y, CONTENT_W, H, {
    bg: C.white, radius: RAD.xxl, stroke: C.slate200, dash: [5, 4],
  });

  const badgeS = 51;                                 // h-12 w-12
  const title = "2GO Travel has no vessels yet";
  const bodyStr = 'Register your fleet to start scheduling voyages, accepting bookings, '
    + 'and tracking capacity. Each vessel needs an IMO number, passenger capacity, '
    + 'and an operational status.';
  const bodyW = 476;                                 // max-w-md
  const bodyLines = Math.max(1, Math.ceil(measure(bodyStr, FS.t12_5) / bodyW));
  const btnH = SP.s2 * 2 + lh(FS.sm);
  ...
  const badge = frame(panel, 'Icon badge', (CONTENT_W - badgeS) / 2, cy, badgeS, badgeS,
    { bg: C.slate100, radius: RAD.full, stroke: C.slate200, strokeOpacity: 0.7 });
  icon(badge, 'Icon', VI.fleet, (badgeS - 25.5) / 2, (badgeS - 25.5) / 2, 25.5, C.slate400);
  cy += badgeS + SP.s4;
  ...
  const b = text(panel, 'Body', bodyStr, (CONTENT_W - bodyW) / 2, cy,
    { size: FS.t12_5, color: C.slate500, lh: FS.t12_5 * RELAXED, width: bodyW, align: 'CENTER' });
  cy += b.height + SP.s4;

  const label = 'Register first vessel';
  const bw = SP.s3_5 * 2 + 12.75 + SP.s1_5 + measure(label, FS.sm, FONT.medium);
  const btn = frame(panel, 'Button - Register first vessel', (CONTENT_W - bw) / 2, cy, bw, btnH,
    { bg: C.brand600, radius: RAD.lg });
  icon(btn, 'Icon', VI.plus, SP.s3_5, (btnH - 12.75) / 2, 12.75, C.white);
  text(btn, 'Label', label, SP.s3_5 + 12.75 + SP.s1_5, SP.s2,
    { size: FS.sm, weight: FONT.medium, color: C.white });
  return panel;
}

/* ── V9b. Page header with the "Add vessel" action ─────────────────────── */

/**
 * The vessels page passes `right`, so PageHeader renders an Add vessel button
 * instead of the default Export. Rebuilt here rather than in the shared chassis,
 * which only knows the Export variant.
 */
function vesselsPageHeaderAction(shell) {
  const header = shell.content.children[0];
  // Drop the shared Export button and put the brand Add vessel button in its place.
  const exportBtn = header.children.filter((c) => c.name === 'Button - Export')[0];
  if (exportBtn) exportBtn.remove();
  const btnH = SP.s1_5 * 2 + lh(FS.sm);
  const label = 'Add vessel';
  const bw = SP.s3_5 * 2 + 12.75 + SP.s1_5 + measure(label, FS.sm, FONT.medium);
  const btn = frame(header, 'Button - Add vessel', CONTENT_W - bw, 0, bw, btnH,
    { bg: C.brand600, radius: RAD.lg });
  icon(btn, 'Icon', VI.plus, SP.s3_5, (btnH - 12.75) / 2, 12.75, C.white);
  text(btn, 'Label', label, SP.s3_5 + 12.75 + SP.s1_5, SP.s1_5,
    { size: FS.sm, weight: FONT.medium, color: C.white });
  return btn;
}

/** buildShell + the vessels-specific header action, in one call. */
function vesselsShell(name, x, y) {
  const s = buildShell(name, x, y, 'Vessels', 'Vessels');
  vesselsPageHeaderAction(s);
  return s;
}

/* ── V10. Frames ───────────────────────────────────────────────────────── */

const V_PAGER_ALL = 'Showing 1–9 of 9 vessels';

// Every dialog frame sits over the same default table, so the chrome behind the
// scrim is identical and the dialogs read as one flow.
function vPageBehind(shell, opts) {
  const o = opts || {};
  return buildVesselsTable(shell.content, shell.bodyY, {
    rows: o.rows || VESSELS, pagerSummary: o.summary || V_PAGER_ALL,
    page: 1, totalPages: 1,
    query: o.query, type: o.type, status: o.status,
    menuRowIndex: o.menuRowIndex, hoverRowIndex: o.hoverRowIndex,
    copiedRowIndex: o.copiedRowIndex, shell: shell,
  });
}
...
const BUILDERS = [
  ...
  { name: 'Vessels / Registered vessels / 01 — View fleet — Loading',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); buildSkeleton(s.content, s.bodyY, 9); } },

  { name: 'Vessels / Registered vessels / 02 — View fleet — Default list',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s); } },

  { name: 'Vessels / Registered vessels / 03 — Search vessel or IMO — Results',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      vPageBehind(s, { rows: VESSELS_SEARCH, query: 'St.',
        summary: 'Showing 1–5 of 5 vessels' }); } },

  { name: 'Vessels / Registered vessels / 04 — Search vessel or IMO — No results',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      vPageBehind(s, { rows: [], query: 'catamaran',
        summary: 'Showing 0–0 of 0 vessels' }); } },

  { name: 'Vessels / Registered vessels / 05 — Filter by type — Menu open',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      const t = vPageBehind(s, { typeOpen: true });
      // Select portals its menu to <body>, so it floats over the card.
      const sel = t.bar.typeSel;
      selectMenu(s.frame, MAIN_X + CONTENT_X + sel.x,
        TOPBAR_H + CONTENT_Y + s.bodyY + sel.y + V_CTRL_H + 4, V_TYPE_W,
        TYPE_OPTIONS, 'All types'); } },

  { name: 'Vessels / Registered vessels / 06 — Filter by status — Menu open',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      const t = vPageBehind(s, { statusOpen: true });
      const sel = t.bar.statusSel;
      selectMenu(s.frame, MAIN_X + CONTENT_X + sel.x,
        TOPBAR_H + CONTENT_Y + s.bodyY + sel.y + V_CTRL_H + 4, V_STATUS_W,
        STATUS_OPTIONS, 'All status'); } },

  // Nine rows push the footer past the 900px fold, so this uses the searched
  // short list — the only way to see the pager on a 1440x900 screen.
  { name: 'Vessels / Registered vessels / 07 — Rows per page — Selector open',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      buildVesselsTable(s.content, s.bodyY, { rows: VESSELS_SEARCH, query: 'St.',
        pagerSummary: 'Showing 1–5 of 5 vessels', page: 1, totalPages: 1,
        sizeOpen: true, shell: s }); } },

  { name: 'Vessels / Registered vessels / 08 — Status pill — Tooltip on hover',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      const i = 4;                                   // MV Our Lady of Akita
      const t = vPageBehind(s, { hoverRowIndex: i });
      const col = t.cols[2];
      const pillW = vStatusPillW(VESSELS[i].st);
      const pillH = lh(FS.t10) + SP.s05 * 2;
      const px = MAIN_X + CONTENT_X + col.x + V_PAD_X;
      const py = TOPBAR_H + CONTENT_Y + s.bodyY + V_TOOLBAR_H + V_THEAD_H
        + i * V_ROW_H + (V_ROW_H - pillH) / 2;
      buildTooltip(s.frame, px + pillW / 2, py, V_STATUS_HINT[VESSELS[i].st]); } },

  { name: 'Vessels / Registered vessels / 09 — Fleet empty — Register first vessel',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      buildFleetEmptyState(s.content, s.bodyY); } },

  /* ── Row actions ───────────────────────────────────────────────────── */

  { name: 'Vessels / Row actions / 01 — Active vessel — Menu open',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      const i = 1;
      vPageBehind(s, { menuRowIndex: i });
      const a = vesselMenuAnchor(s, i);
      buildVesselMenu(s.frame, a.x, a.y, vesselMenuItems('Active')); } },

  { name: 'Vessels / Row actions / 02 — Maintenance vessel — Menu open',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      const i = 4;
      vPageBehind(s, { menuRowIndex: i });
      const a = vesselMenuAnchor(s, i);
      buildVesselMenu(s.frame, a.x, a.y, vesselMenuItems('Maintenance')); } },

  { name: 'Vessels / Row actions / 03 — Retired vessel — Menu open',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      const i = 7;
      vPageBehind(s, { menuRowIndex: i });
      const a = vesselMenuAnchor(s, i);
      buildVesselMenu(s.frame, a.x, a.y, vesselMenuItems('Retired')); } },

  /* ── IMO ───────────────────────────────────────────────────────────── */

  { name: 'Vessels / IMO / 01 — Copy IMO — Copied',
    build: (x, y, n) => { const s = vesselsShell(n, x, y);
      vPageBehind(s, { copiedRowIndex: 0 });
      buildToast(s.frame, 'IMO 9756101 copied'); } },

  /* ── Add vessel ────────────────────────────────────────────────────── */

  { name: 'Vessels / Add vessel / 01 — Empty form — Register disabled',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      buildVesselFormDialog(s.frame, { mode: 'add' }); } },

  { name: 'Vessels / Add vessel / 02 — Fast Craft picked — Ready to register',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      buildVesselFormDialog(s.frame, { mode: 'add', name: 'FC Camotes Flyer',
        imo: '9812340', type: 'Fast Craft', status: 'Active' }); } },

  // The form is ~42px taller than max-h-[60vh] allows, so Status only appears
  // once the body is scrolled. Same dialog, scrolled to the end.
  { name: 'Vessels / Add vessel / 03 — Scrolled to Status — Below the fold',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      buildVesselFormDialog(s.frame, { mode: 'add', name: 'FC Camotes Flyer',
        imo: '9812340', type: 'Fast Craft', status: 'Inactive', scroll: 'bottom' }); } },

  { name: 'Vessels / Add vessel / 04 — Vessel added — Toast',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      buildToast(s.frame, 'FC Camotes Flyer added to 2GO Travel'); } },

  /* ── Edit vessel ───────────────────────────────────────────────────── */

  { name: 'Vessels / Edit vessel / 01 — Dialog open',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      buildVesselFormDialog(s.frame, { mode: 'edit', name: 'MV Palawan Breeze',
        imo: '9756101', type: 'RoRo', status: 'Active' }); } },

  { name: 'Vessels / Edit vessel / 02 — Vessel updated — Toast',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      buildToast(s.frame, 'MV Palawan Breeze updated'); } },

  /* ── Vessel status ─────────────────────────────────────────────────── */

  { name: 'Vessels / Vessel status / 01 — Disable vessel — Confirm dialog',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      buildVesselStatusDialog(s.frame, true); } },

  { name: 'Vessels / Vessel status / 02 — Enable vessel — Confirm dialog',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      buildVesselStatusDialog(s.frame, false); } },

  { name: 'Vessels / Vessel status / 03 — Vessel disabled — Toast',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      // showToast(…, "error") — the disable path uses the red variant.
      buildToast(s.frame, 'MV Visayan Star disabled', 'error'); } },

  { name: 'Vessels / Vessel status / 04 — Vessel activated — Toast',
    build: (x, y, n) => { const s = vesselsShell(n, x, y); vPageBehind(s);
      buildToast(s.frame, 'MV Our Lady of Akita activated'); } },
];

/* ── V11. Run — creates its own section below everything on the page ───── */

const SECTION_NAME = 'Vessels — All states';

async function main() {
  FONT = await loadFonts();

  // Anchor to the page that already holds the Tickets/Routes sections.
  let page = null;
  try {
