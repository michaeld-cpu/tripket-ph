/* ============================================================================
 * Tripket — Build "Reports — All states"
 * ----------------------------------------------------------------------------
 * Rebuilds app/reports/page.tsx as native Figma frames. The route currently
 * renders <ComingSoon /> and nothing else, so this section holds exactly one
 * frame. The plugin CREATES its own section below every existing section on
 * the page and is ADDITIVE: a frame whose name already exists is skipped.
 *
 * Shares the chrome layer (sidebar, topbar, embedded logos, text measurement)
 * byte-for-byte with the Routes / Vessels / Bookings / Tickets / Shipping-lines
 * plugins. Reports-only code starts at the "P1." marker.
 *
 * Measurements are real CSS pixels at 1440x900. globals.css sets
 * html { font-size: 17px }, so rem utilities are 17px-based (px-8 = 34,
 * py-16 = 68). Its type-scale layer bumps a FIXED LIST of arbitrary
 * text-[Npx] values by ~1px — text-[26px] is not on that list and renders at 26.
 * Literal px classes (h-[72px], min-h-[60vh]) do NOT scale.
 * ========================================================================== */
...
/* ============================================================================
 * P1. Reports — the ComingSoon placeholder
 * ----------------------------------------------------------------------------
 * Sources: app/reports/page.tsx (13 lines), components/ComingSoon.tsx,
 * components/AppChrome.tsx (the <main> padding).
 *
 * app/reports/page.tsx renders `<ComingSoon />` and nothing else — no
 * PageHeader, no toolbar, no table. One route, one state, one frame.
 * ========================================================================== */

// text-[26px] is NOT in the globals.css type-scale layer (which only lists
// 9.5 / 10 / 10.5 / 11 / 11.5 / 12 / 12.5 / 13 / 13.5 / 14 / 15 / 17), so it
// renders at its literal 26px. text-[17px] IS listed → 18px (FS.t17).
const CS_TITLE = 26;

// ComingSoon: h-[72px] w-[72px] literal, h-9 w-9 glyph (rem → 38.25).
const CS_MARK = 72;
const CS_GLYPH = SP.s9;

// Clock with a dashed arc — verbatim path data, strokeWidth 1.75.
const CS_ICON = {
  sw: 1.75,
  d: '<path d="M12 7v5l3 2"/>' +
     '<path d="M21 12a9 9 0 1 1-6.2-8.6" stroke-dasharray="2.5 3"/>' +
     '<path d="M21 12a9 9 0 0 0-3.5-7.1"/>',
};

const CS_DEFAULT_TITLE = 'Coming Soon';
const CS_DEFAULT_MESSAGE = 'This feature is coming soon. Stay tuned!';

/**
 * `grid min-h-[60vh] place-items-center px-6 py-16`
 *
 * Tailwind's preflight sets box-sizing: border-box, so min-h-[60vh] (540 at
 * 900vh) includes the py-16 padding: the grid's content box is 540 - 136 = 404
 * and the centred item is centred in that, not in the visible <main>.
 */
function buildComingSoon(parent, x, y, w, title, message) {
  const PAD_X = SP.s6;                       // px-6
  const PAD_Y = 68;                          // py-16 → 16 * 4.25
  const MIN_H = FRAME_H * 0.6;               // min-h-[60vh]

  const markH = CS_MARK;
  const titleH = lh(CS_TITLE);
  const msgH = lh(FS.t17);
  const itemH = markH + SP.s5 + titleH + SP.s1_5 + msgH;   // mt-5, mt-1.5
  const boxH = Math.max(MIN_H, PAD_Y * 2 + itemH);

  const box = frame(parent, 'ComingSoon', x, y, w, boxH);
  const cx = w / 2;                                        // place-items-center
  const itemY = PAD_Y + (boxH - PAD_Y * 2 - itemH) / 2;

  const mark = frame(box, 'Mark', cx - CS_MARK / 2, itemY, CS_MARK, CS_MARK,
    { bg: C.brand50, radius: RAD.full });
  icon(mark, 'Icon · clock', CS_ICON, (CS_MARK - CS_GLYPH) / 2, (CS_MARK - CS_GLYPH) / 2,
    CS_GLYPH, C.brand500);

  const h2 = text(box, 'Title', title, 0, itemY + markH + SP.s5,
    { size: CS_TITLE, weight: FONT.semibold, color: C.slate600, tracking: -0.65 });
  h2.x = cx - h2.width / 2;

  const p = text(box, 'Message', message, 0, itemY + markH + SP.s5 + titleH + SP.s1_5,
    { size: FS.t17, color: C.slate400 });
  p.x = cx - p.width / 2;

  return box;
}

/** Shell without a PageHeader — app/reports/page.tsx renders none. */
function reportsShell(name, x, y) {
  const f = frame(figma.currentPage, name, x, y, FRAME_W, FRAME_H, { bg: C.white, clip: true });
  LAST_SHELL = f;
  buildSidebar(f, 'Reports');
  const right = frame(f, 'Container', MAIN_X, 0, MAIN_W, FRAME_H);
  buildTopbar(right);
  const main = frame(right, 'Main Content', 0, TOPBAR_H, MAIN_W, MAIN_H, { clip: true });
  // <main className="flex-1 overflow-y-auto px-8 py-6">
  const content = frame(main, 'Container', CONTENT_X, CONTENT_Y, CONTENT_W, MAIN_H - CONTENT_Y * 2);
  return { frame: f, container: right, content: content };
}
...
const BUILDERS = [
  {
    name: 'Reports / 01 — Coming Soon',
    build: (x, y, n) => {
      const s = reportsShell(n, x, y);
      buildComingSoon(s.content, 0, 0, CONTENT_W, CS_DEFAULT_TITLE, CS_DEFAULT_MESSAGE);
    },
  },
];
...
const SECTION_NAME = 'Reports — All states';
...
async function main() {
  ...
  const cols = Math.min(BUILDERS.length, GRID_COLS);
  const needW = GRID_MARGIN_X * 2 + cols * FRAME_W + (cols - 1) * GRID_GAP;
